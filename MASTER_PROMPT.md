...
12. **The human operator approves phases directly — you are not waiting on any third party.** The human may be offline or unavailable for extended periods and will approve each phase themselves, in real time, based solely on your result file and the completion announcement in §4a below. You must make that announcement unambiguous and self-contained enough that a non-expert human, reading only that announcement and the result file, can confidently decide to say "proceed." Do not imply, hint at, or wait for any external review beyond the human operator's own "proceed to Phase N" instruction — that instruction alone is sufficient authorization to continue. The human may later share result files with a separate reviewer for retrospective cross-checking; this happens asynchronously and is never a precondition for you to proceed. Your only gate is the human operator's explicit "proceed" message in this chat.
...
## 4a. Mandatory completion announcement (separate from the result file, posted in the chat/IDE output itself)

Immediately after saving a phase's result file with `STATUS: READY FOR REVIEW`, post this exact announcement, filled in, as your own message — do not bury it inside the file, and do not add extra hedging or caveats beyond what the template asks for:

```
✅ PHASE NN COMPLETE — 100%
Name: <phase name>
Result file saved at: /phase_results/phaseNN_<name>_report.md
All required tests: <count> / <count> PASSING
Blockers: NONE  (or: BLOCKED — see result file for exact unblock steps)
Deviations from spec: NONE  (or: <one-line summary — full detail in result file>)

Waiting for your instruction: reply "proceed to Phase <NN+1>" to continue,
or ask me to fix/re-verify anything first.
```

If `STATUS: BLOCKED`, post the equivalent blocked version instead, with the exact manual unblock steps inline in the chat message itself (not just in the file), so the human can act on it immediately without opening the file.

This announcement is what the human operator uses to approve the phase themselves, in real time, without needing anyone else to review it first. Never say "complete" or "100%" here unless every requirement in that phase's checklist is genuinely checked off with real evidence — this message is a direct authorization trigger, so treat it with the same rigor as the result file itself.
